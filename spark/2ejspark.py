...
def tiempo_entre_conexiones(IP,valores):
    ...
    for fecha_dada,lista,conn_realizadas in valores:
        ...
        return (tuple((IP,lista)),list((diferencia_fecha,fecha_dada,conn_realizadas,1)))
...
def comportamientos_repetidos(clave,valores):
    ...
    clAve=list(clave)
    #return valores
    for (diferencia_fecha,fecha_dada,conn_realizadas,repeticion_en_sesion) in valores:
        i=i+1
...
if __name__=="__main__":
    ...
    ipsGood = ips.filter(lambda x: x[8]==('200')).sortBy(lambda x: x[3]).map(lambda x: (fecha_de_sesion(x))).groupByKey()

    ips2 = ipsGood.map(lambda x: accesos_en_sesion(x[0],x[1]))
    ips3 = ips2.groupByKey()
    ips4 = ips3.map(lambda x: tiempo_entre_conexiones(x[0],x[1]))
    #ips5 = ips4.map(lambda (x,y): (tuple(x),y)).groupByKey()
    ips6 = ips4.map(lambda x: comportamientos_repetidos(x[0],x[1]))
    #ips7 = ips6.collect()
            
    #ips6.saveAsTextFile("salida-ejemplo2")
    for x in ips6.collect():
         print (":"+str(x)+":")
         #print (":"+str(x[0][0])+":-->: "+str(len(x[0][1]))+":")
